...
function xNzq (a){
    ...
    const uniquePages = a.reduce((acc, current) => {
        // Używamy `strona` jako klucza dla unikalności
        const isDuplicate = acc.find(item => item.strona === current.strona);
        if (!isDuplicate) {
            acc.push(current);
        }
        return acc;
    ...
    // Sortujemy po numerze strony
    uniquePages.sort((a, b) => a.strona - b.strona);

    // Zwracamy posortowaną tablicę bez duplikatów
    return uniquePages;
}
...
function detektor(dlugiCiag, wzor) {
    let count = 0;
    let position = dlugiCiag.indexOf(wzor);

    while (position !== -1) {
        count++;
        position = dlugiCiag.indexOf(wzor, position + 1);
    }
